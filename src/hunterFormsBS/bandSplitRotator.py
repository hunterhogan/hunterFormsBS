"""Use the unified BS-RoFormer and Mel-Band RoFormer waveform separator.

You can use this module to instantiate one configurable waveform-to-waveform music source separator
that unifies the non-overlapping band-split front end from BS-RoFormer [1] and the overlapped mel-band
front end from Mel-Band RoFormer [2]. The module exposes one public class with a forward method
suitable for both inference and supervised training.
...
`sample_rate=44100`, `stft_n_fft=2048`, and `num_bands=60`. If you need a different mel-band or
non-overlapping layout, pass `mask_filter_bank` explicitly. The helper module
`hunterFormsBS.make_static_mask_filter_bank` can print static `mask_filter_bank` definitions for
custom mel-band or non-overlapping layouts.

Contents
--------
Classes
	BandSplitRotator
		Unified waveform separator with configurable band front end, RoPE or PoPE positional
		embedding, optional linear-attention pre-block, and multi-resolution STFT training loss.
...
class BandSplitRotator(Module):
	...
	loss through `hunterFormsBS.bandSplit.lossComputation` [5]. The class can follow the
	non-overlapping band-split front end from BS-RoFormer [1] or the overlapped mel-band front end
	from Mel-Band RoFormer [2].
	...
	def __init__(
		...
		*,
		...
		you get a size-mismatch error when loading the checkpoint, check the `mask_estimator_depth`
		value in the configuration file. The original `BSRoFormer` implementation used
		`mask_estimator_depth=2` and the logic later subtracted one from the value. This package
		eliminates the subtraction, so the effective default is `mask_estimator_depth=1`. Change your
		configuration file to set `mask_estimator_depth=1`, and the size-mismatch error should be
		resolved.
		...
		if mask_filter_bank is None:
			if (sample_rate is not None) and (num_bands is not None):
				# Tantamount to `MelBandRoformer = True`.  # noqa: ERA001
			...
			num_bands = mask_filter_bank.shape[0]
	...
	def forward(self, raw_audio: Tensor, target: Tensor | None = None, active_stem_ids: list[int] | None = None, *, return_loss_breakdown: bool = False) -> Tensor | tuple[Tensor, tuple[Tensor, Tensor]]:
		...
		hierarchical attention core : behavior
			...
			whether the non-linear attention block receives RoPE [3] or PoPE [4] or NoPE or CoPE.

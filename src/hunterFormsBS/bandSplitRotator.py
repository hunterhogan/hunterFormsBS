...
Classes
	BandSplitRotator
		Unified waveform separator with converged constructor arguments, optional `SageAttention`
...
class BandSplitRotator(Module):
	...
	def __init__(
		...
		"""Configure the converged separator and its band-split front end.
...
#======== class attributes, including backward compatibility =========
		...
		if mask_filter_bank is None:
			if (num_bands is not None) and (sample_rate is not None):
				...
				# Tantamount to `BSRoformer = True`, by default.
...
#======== band split and mask estimator ===================
	...
	def forward(self, raw_audio: Tensor, target: Tensor | None = None, active_stem_ids: list[int] | None = None, *, return_loss_breakdown: bool = False) -> Tensor | tuple[Tensor, tuple[Tensor, Tensor]]:
		...
		stft_window: Tensor = self.stft_window_fn(device=raw_audio.device)
		...
		try:
			...
			stft_repr = torch.stft(raw_audio.cpu(), **self.stft_kwargs, window=stft_window.cpu(), return_complex=True).to(raw_audio.device)
		...
		batch_arange: Tensor = torch.arange(batch, device=raw_audio.device)[..., None]
		...
		store: list[Tensor] = [tensor([])] * len(self.layers)
		...
		if active_stem_ids is None:
			...
			heads = ModuleList([self.mask_estimators[stem_id] for stem_id in active_stem_ids])
		...
		# average the estimated mask for the overlapped frequencies
		...
		if self.zero_dc:
			stft_repr = stft_repr.index_fill(1, tensor(0, device=raw_audio.device), 0.0)
		...
		try:
			...
			recon_audio = cast('Callable[..., Tensor]', torch.istft)(stft_repr.cpu(), **self.stft_kwargs, window=stft_window.cpu(), return_complex=False, length=istft_length).to(raw_audio.device)  # pyright: ignore[reportUnknownMemberType]

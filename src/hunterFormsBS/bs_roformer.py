...
with the non-overlapping band-split front end from BS-RoFormer [1]. The module exposes one public
class with a forward method suitable for both inference and supervised training.
...
class BSRoformer(Module):
	...
	loss through `hunterFormsBS.bandSplit.lossComputation` [5]. The class follows the non-overlapping
	band-split front end from BS-RoFormer [1].
	...
	automatic front-end selection : behavior
		When `mask_filter_bank` is `None`, the constructor derives the non-overlapping band layout
		from `freqs_per_bands`, matching `BSRoFormer` [1].
